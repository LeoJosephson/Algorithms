...
class Solution {
  public:
    ...
    int assignmentProblem(int N, int R) {
        ...
        for(int i=1; i<=n; i++){
        	int x, y;
        	cin >> x >> y;
        	PXR.push_back(x); PYR.push_back(y);
		}
		for(int i=1; i<=N; i++){
        	int x, y;
        	cin >> x >> y;
        	PXN.push_back(x); PYN.push_back(y);
		}
		
		for(int i=0; i<n; i++){
			for(int j=0; j<N; j++){
				Arr[i*n+j] = abs(PXR[i]-PXN[j]) + abs(PYR[i]-PYN[j]);
			}
		}
...
{   	
	...
	while(cin >> r >> n){
		Solution ob;
		cout<<ob.assignmentProblem(n, r)<<endl;	
	}															                                                              
